    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-accent-dim/40 bg-accent/[0.06] px-4 py-3">
        ...
        <div className="mt-2.5 flex flex-wrap items-center gap-2 font-mono text-[0.65rem]">
          ...
          <span className="rounded bg-panel-2 px-1.5 py-0.5 text-accent ring-1 ring-edge">
            {phase.produces}
          </span>
...
export default function EngineAnatomy() {
  ...
  return (
    <>
      ...
      <p>
        For a standard (non-MLA) transformer layer, one block's size in bytes is:
      </p>
      <CodeBlock
        lang="text"
      ...
      <h3>Worker startup: three procedures</h3>
      <p>
        Constructing the model executor creates a <Code>Worker</Code> and runs three procedures.
        These same three will later run independently on every worker process across every GPU —
        so it is worth knowing them by name.
      </p>
      <div className="my-5 grid gap-3 sm:grid-cols-3">
        {[
          ...
          <Card key={x.n} className="p-3.5">
            <div className="font-mono text-[0.6rem] text-accent/70">{x.n}</div>
      ...
      <Callout kind="note" title="Where the KV cache size comes from">
        <p>
          Notice that nobody configures the number of KV blocks directly. Step 03 measures it: run
          a dummy forward pass, see how much VRAM is left, divide by block size. That is why
          changing <Code>gpu_memory_utilization</Code>, the model, or the dtype silently changes how
          many requests you can hold in flight — and therefore your throughput ceiling.
        </p>
      </Callout>
      ...
      <SimFrame
        sim={sim}
        keys
        title="One request through the whole engine"
        subtitle="Highlighted node = the component currently doing work. The dashed edges are the ones that only matter later: freed blocks returning to the pool, and the grammar bitmask reaching into sampling."
      >
        <EngineViz sim={sim} />
      </SimFrame>
