          ...
          return (
            ...
            >
              ...
              <div className="mt-2 flex min-h-7 flex-wrap gap-1.5">
                {members.length === 0 ? (
                  <span className="font-mono text-[0.62rem] text-ink-faint/50">idle</span>
                ) : (
                  members.map((r) => (
                    <span
                      key={r.id}
                      className="rounded px-1.5 py-0.5 font-mono text-[0.62rem]"
                      style={{ background: reqColor(r.idx), color: '#08090d' }}
        ...
        >
          ...
          <div className="mt-2 flex min-h-7 flex-wrap items-center gap-1.5">
            {state.store.length === 0 ? (
              <span className="font-mono text-[0.62rem] text-ink-faint/50">empty</span>
            ) : (
              state.store.map((id) => {
                ...
                return (
                  <span
                    key={id}
                    className="rounded px-1.5 py-0.5 font-mono text-[0.62rem]"
                    style={{ background: C.warn, color: '#08090d' }}
      ...
      <div>
        ...
        <div className="scroll-x rounded-md border border-edge bg-[#08090d] p-3">
          <div className="flex min-w-max items-end gap-[3px]" style={{ height: 110 }}>
            {state.steps.map((x, i) => (
              <div key={i} className="flex w-4 flex-col justify-end gap-[2px]" style={{ height: '100%' }}>
                {x.prefillTokens > 0 && (
                  <div
      ...
      <div className="space-y-1">
        {state.requests.map((r) => (
          <div key={r.id} className="flex items-center gap-2 font-mono text-[0.65rem]">
            ...
            <span className="w-20 text-[0.58rem] text-ink-faint tabular-nums">
              p{r.promptLen}
            </span>
  ...
  return (
    <>
      ...
      <SimFrame
        sim={sim}
        keys
        title="KV handoff between instances"
        subtitle="Requests arrive in bursts. Colocated, prefills and decodes share every step; disaggregated, they run on separate GPUs in parallel and KV moves over the wire in between."
        legend={[
        ...
        footer={
          <>
            ...
            token can be produced. Turn up the transfer cost and watch disaggregation stop being
            worth it: this is why the connector implementation matters so much in practice.
          </>
        }
      >
        <PdViz sim={sim} />
      ...
      <BlogFigure src="pd.png" caption="Disaggregated prefill/decode with a KV-cache service between" />
      ...
      <Callout kind="key" title="The same seam as prefix caching">
        <p>
          ...
          <em>local</em> prefix-cache check, and its result is added to the local computed-token count
          before <Code>allocate_slots</Code>. Disaggregated P/D is, structurally, prefix caching with
          the cache living on another machine. Stage 07's machinery is doing the work; the connector
          just widens where a "hit" can come from.
        </p>
      </Callout>
    ...
    outputs = llm.generate(prompts, sampling_params)   # fetches KV first`}
      ...
      <Callout kind="note" title="Details that matter in production">
        <ul>
          ...
          <li>
            <Code>LMCache</Code> (backed by NVIDIA's NIXL) is the fastest production-ready connector,
            though it lives at the bleeding edge.
          </li>
        </ul>
      </Callout>
