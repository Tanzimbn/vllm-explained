        ...
        <div className="flex flex-wrap gap-2">
          {state.slots.map((idx, si) => {
            ...
            return (
              <div
                key={si}
                className="flex h-14 w-24 flex-col items-center justify-center rounded-lg border font-mono text-[0.68rem] transition-colors duration-300"
                style={{
                  borderColor: r ? (finished ? C.edge : C.decode) : C.edge,
                  background: r
                    ? finished
              ...
              >
                {r ? (
                  <>
                    <span style={{ color: finished ? C.faint : C.decode }}>{r.id}</span>
                    <span className="text-[0.6rem] text-ink-faint">
                      {finished ? 'idle — held' : `${r.generated}/${r.outLen} tok`}
                    </span>
                  </>
                ) : (
                  <span className="text-ink-faint/50">free</span>
                )}
              </div>
            )
          })}
  ...
  return (
    <>
      <p>
        ...
        hardware. Getting them to share one GPU efficiently is the problem that shapes every
        design decision in the rest of this roadmap.
      </p>

      <h3>The two workloads</h3>
      <p>
      ...
      <p>
        ...
        chip to compute that one token. Decode is <strong>memory-bandwidth-bound</strong>, and it
        is wildly inefficient per token: you move gigabytes to produce a handful of bytes.
      </p>
      ...
      <Callout kind="key">
        <p>
          ...
          expensive weight-streaming pass gets amortized across as many tokens as possible. And
          both need to happen on the same GPU, interleaved, without either starving the other.
        </p>
      </Callout>
      ...
      <p>
        ...
        naive way to do that is <strong>static batching</strong> — collect <Code>B</Code>{' '}
        requests, run them together, return all the results, collect the next <Code>B</Code>.
      </p>
      <p>
        ...
        in the batch doing nothing while new requests queue up outside. Run the simulator below in{' '}
        <Code>static</Code> mode and watch the slot utilization number — then flip it to{' '}
      ...
      <SimFrame
        ...
        subtitle="One tick is one engine step. Each request's first step is its prefill; the rest are decodes. Turn up the output-length spread to make the effect brutal."
        legend={[
          { label: 'prefill step', color: C.prefill },
          { label: 'decode step', color: C.decode },
          { label: 'queued outside the batch', color: C.free },
          { label: 'finished', color: C.good },
        ]}
        footer={
          <>
            Notice what changes and what doesn't: continuous batching does not make any single
            forward pass faster. It just stops you from paying for capacity you aren't using —
            which is why the wasted-slot-step counter, not the tick counter, is the one to watch.
          </>
        }
      >
        <BatchingViz sim={sim} />
      ...
      <p>
        ...
        stacking sequences into a padded rectangle, vLLM concatenates them into one long flat
        "super sequence", with position indices and custom attention kernels making sure each
        sequence only attends to its own tokens. There is no rectangle to keep intact, so there is
        nothing stopping the batch composition from changing every step.
      </p>
      ...
      <Callout kind="gotcha" title="Offline vs online">
        <p>
          ...
          batching becomes visible with the <em>asynchronous</em> engine, where requests arrive
          over the network at arbitrary times. But the underlying capability is in the engine core
          either way, because of that flattened-batch design.
        </p>
      </Callout>
